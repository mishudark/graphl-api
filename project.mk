...
bazel-test: ## Test
	bazel $(BAZEL_OUTPUT) shutdown
	bazel $(BAZEL_OUTPUT) test $(BAZEL_REPOSITORY) $(BAZEL_FLAGS) $(BAZEL_REMOTE) $(BAZEL_BUILDKITE) //pkg/...

bazel-dist: ## Build the project
	bazel run --distdir=./distfiles2/ $(BAZEL_REPOSITORY) $(BAZEL_FLAGS) $(BAZEL_REMOTE) :additional_distfiles
...
bazel-build: ## Run the project inside docker
	bazel $(BAZEL_OUTPUT) build $(BAZEL_REPOSITORY) $(BAZEL_FLAGS) $(BAZEL_REMOTE) $(BAZEL_BUILDKITE_BUILD) //cmd/graphql:docker

bazel-lint:
	bazel $(BAZEL_OUTPUT) run $(BAZEL_REPOSITORY) $(BAZEL_FLAGS) $(BAZEL_REMOTE) $(BAZEL_BUILDKITE_BUILD) //:golangcilint
